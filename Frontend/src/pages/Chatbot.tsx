...
const Chatbot = () => {
  ...
  const analysisTags: AnalysisTag[] = [
    {
      id: 'Nombre Totale des beneficiaires',
      label: 'Nombre Totale des Bénéficiaires (%) ',
      query: 'Calcule le nombre Totale des Bénéficiaires de ce fichier  '
    },
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/chat`, {
        ...
        body: JSON.stringify({
          ...
          timestamp: Date.now() // Ajouter ce paramètre pour éviter la mise en cache
        }),
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (data.success && (data.data || data.fullData)) {
          ...
          setMessages(prev => [...prev, {
            sender: 'bot',
            text: `Le fichier ${file.name} a été téléchargé avec succès ${fileSizeInfo}.`,
            id: Date.now()
          }]);
        } else {
  ...
  return (
    ...
    {/* Conteneur principal */}
    <div className="mt-auto flex-1 flex flex-col items-center justify-between py-4 px-4">
      ...
      <div 
        ...
        style={{
          ...
          maxHeight: 'calc(100vh - 300px)' // Ajusté pour tenir compte de la section des tags
        }}
      >
        <style>
          {`
      ...
      {/* Tags d'analyses prédéfinies - Juste au-dessus de la zone de saisie */}
      <div className="w-full flex flex-col-reverse">
        <div className="flex overflow-x-auto space-x-2 pt-2 no-scrollbar">
          {analysisTags.map(tag => (
            <button
              key={tag.id}
              onClick={() => applyAnalysisTag(tag)}
              className="bg-white/50 rounded-xl p-3 shadow-sm border border-orange-300 text-xs min-w-[120px] text-left flex-shrink-0"
