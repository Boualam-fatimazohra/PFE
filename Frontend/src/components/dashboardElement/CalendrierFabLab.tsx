...
import { useFab } from "@/contexts/FablabContext"; 
...
const CalendrierFablab = () => {
  ...
  const { FabFormations, loading, fetchFabFormations } = useFab();
  ...
  useEffect(() => {
    ...
    if (FabFormations && FabFormations.length > 0) {
      ...
      if (showFormations) {
        ...
        const formationEvents = FabFormations.map(formation => ({
...
export default CalendrierFablab;