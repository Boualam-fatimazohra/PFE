...
// Define FormateurItem interface to match what's expected
...
const Ecolecode = () => {
  ...
  // Add missing properties to match the Formation type
  const transformToFormation = (formation: any): Formation => {
    return {
      _id: formation._id || "",
      nom: formation.nom || "",
      ...
      image: formation.image || "/api/placeholder/60/60",
      formateur: formation.formateur || "",
      // Add missing properties required by Formation type
      lienInscription: formation.lienInscription || "",
      tags: formation.tags || [] 
  ...
  // Make sure we properly transform the data to match the Formation type
  const mappedFormationsAsFormation: Formation[] = formationData.map((item, index) => transformToFormation({
    _id: item._id || index.toString(),
    nom: item.nom,
    title: item.nom,
    dateDebut: item.dateDebut,
    dateFin: item.dateFin,
    status: item.status as "En Cours" | "Avenir" | "Terminé" | "Replanifier",
    image: item.image,
    formateur: item.formateurName || ""
  }));

  // Transform formateur data to match FormateurItem interface
  const formateurData: FormateurItem[] = (edcFormateurs && edcFormateurs.length > 0
    ? edcFormateurs.map((formateur, index) => ({
        id: formateur._id || index.toString(),
    ...
    : [
        { id: "1", nom: "Sophie Martin", role: "formateur", ville: "Rabat", disponible: true, enFormation: false, formations: 12, beneficiaires: 184, satisfaction: "4.8/5" },
        { id: "2", nom: "Thomas Dubois", role: "formateur", ville: "Casablanca", disponible: true, enFormation: true, formations: 8, beneficiaires: 126, satisfaction: "4.7/5" },
        { id: "3", nom: "Amina Benali", role: "formateur", ville: "Agadir", disponible: true, enFormation: false, formations: 10, beneficiaires: 152, satisfaction: "4.9/5" },
      ]) as FormateurItem[];
        ...
        <Tabs defaultValue="formations" className="mb-6" onValueChange={setActiveTab}>
          ...
          <TabsContent value="formations" className="space-y-6">
            ...
            <Card className="border-gray-200 rounded-md shadow-sm">
              <CardContent className="p-0">
                <FormationsTable
                  ...
                  formations={mappedFormationsAsFormation} // Pass the correctly typed formations
                />
              </CardContent>
            </Card>
            ...
            <Card className="border-gray-200 rounded-md shadow-sm">
              <CardContent className="p-0">
                <FormateurList 
                  formateurs={formateurData} // Pass the correctly typed formateurs
                />
              </CardContent>
            </Card>
          </TabsContent>
