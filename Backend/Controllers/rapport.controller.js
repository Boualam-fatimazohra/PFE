const {analyserBeneficiaires ,getFormationsTermineesByFormateurs} =require("../utils/rapportUtils");
const {getBeneficiairesByFormation} =require("../utils/BeneficiairePresence");
const createRapport = async (req, res) => {
    console.log("Create rapport");
    try {
      const { dateDebut, dateFin, formateurIds } = req.body;
      
      // Validation des entrées
      if (!dateDebut || !dateFin || !formateurIds) {
        return res.status(400).json({
          success: false,
          message: "Les dates de début et fin, ainsi que les IDs des formateurs sont obligatoires"
        });
      }
